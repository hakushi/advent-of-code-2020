...
module.exports = data.split('\n').map(row => {
    const obj = {
        colorType:row.split(' bags')[0].split(' ')[0],
        mainColor: row.split(' bags')[0].split(' ')[1],
        contents: 
            row
                .split('contain ')[1].split(', ')
                .map(row => ({
                    amount: parseInt(row.split(' ')[0], 10),
                    colorType: row.split(' ')[1].split(' bag')[0],    
                    mainColor: row.split(' ')[2].split(' bag')[0]    
                }))
            }
    return obj;
});
